...
const createUsers = async function (req, res) {
    ...
    try {
        ...
        if (!confirmPassword) return res.status(400).send({ status: false, message: "confirmPassword is mandatory" })
        if (confirmPassword !== password) {
            return res.status(400).send({ status: false, message: "password must match" })
        }
...
const getUser = async (req, res) => {
    try{
     ...
     return res.status(200).send({status:true,message:"got user successfully",data:user})
    }
    catch (err) {
        return res.status(500).send({ status: false, message: err.message })
    }
